...
gcloud dataflow flex-template run "real-time-flow-weather-bit-`date +%Y%m%d-%H%M%S`" \
 --template-file-gcs-location "gs://cloud-computing-3454685-dataflow-real-time-temp-bucket/real-time-flow-py.json" \
 --parameters=project_id="cloud-computing-3454685",pubsub_subscription="projects/cloud-computing-3454685/subscriptions/WeatherBit-realtime-subscription",bigtable_instance_id="real-time-bigtable",bigtable_table_id="WeatherBit" \
 --region "europe-central2" \
--additional-experiments=streaming_mode_exactly_once