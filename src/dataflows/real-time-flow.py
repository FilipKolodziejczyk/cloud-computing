...
def run(argv=None, save_main_session=True):
    ...
    parser.add_argument(
        "--pubsub_subscription",
        help="Pub/Sub subscription ID (format projects/<project>/subscriptions/<subscription>).",
    )
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        transformed_messages = parsed_messages | "Transform Data" >> beam.Map(
            lambda x: x
        )  # Replace this with transformation logic

        direct_rows = transformed_messages | "Create DirectRow" >> beam.ParDo(
            CreateDirectRow()
        )
